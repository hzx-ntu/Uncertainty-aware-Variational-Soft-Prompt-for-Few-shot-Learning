...
class BaseEmbedder(torch.nn.Module):
    def __init__(self,
        ...
        loss_para_1: float = 0.5, #hzx added
        loss_para_2: float = 0.5, #hzx added
        **kwargs
        ) -> None:
        super().__init__()
        ...
        self.dropout = dropout
        self.xe_loss = torch.nn.CrossEntropyLoss(reduction='none') #hzx modified, default is 'mean'
        ...
        #hzx added
        self.loss_para_1 = loss_para_1
        self.loss_para_2 = loss_para_2
    ...
    def decoding_loss(
        ...
        **kwargs
        ...
        if prompt_var is not None:
            ...
            kl_loss = -0.5 * torch.mean(1 + prompt_var - prompt_var.exp(), axis = -1)
            
            xe_loss = self.xe_loss(
                    input=decoding_logits,
                    target=labels.to(dtype=torch.long))

            decoding_loss = xe_loss * torch.mean(1.0 / prompt_var.exp(), axis = -1) + self.loss_para_1 * (1 - self.loss_para_2 * mask_bold) * kl_loss
            
            return {
                'decoding_loss': torch.mean(decoding_loss)
            }
        return {
