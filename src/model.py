...
class Model(torch.nn.Module):
    ...
    def __init__(
        ...
        softprompt: torch.nn.Module = None,
        unembedder: torch.nn.Module = None,
        shadowembed: torch.nn.Module = None,
        shadowdecoder: torch.nn.Module = None,
        is_prompt_norm: bool = False
        ) -> torch.nn.Module:
        ...
        self.softprompt = softprompt #hzx added
        self.shadowembed = shadowembed #hzx added
        self.shadowdecoder = shadowdecoder #hzx added
        self.is_decoding_mode = False
        
        self.is_prompt_norm = is_prompt_norm #hzx added
        if self.is_prompt_norm:
           print('Normlize the weights of soft prompt...')
    ...
    def compute_loss(
        ...
        prompt_weights = {}
        if self.is_prompt_norm:
           for param_name,param in self.softprompt.named_parameters():
               if 'weight' in param_name:
                  prompt_weights[param_name] = torch.norm(param, p=2)
           l2_norm = sum(prompt_weights.values())
           losses['loss'] += -0.01*l2_norm
    ...
    def prep_batch(
        ...
        batch: Dict[str, torch.tensor]
            Input batch (as generated by src.batcher)
        """
        return self.embedder.prep_batch(batch=dict(batch),prompt_embed=prompt_embed)
    ...
    def forward(
        ...
        #hzx added
        prompt_embed = None
        prompt_input_var = None
        prompt_input_mu = None
        if self.softprompt is not None:
           prompt_input, prompt_input_var, prompt_input_mu = self.softprompt(batch=batch)
           #prompt_input = self.softprompt(batch=batch)
           prompt_embed = prompt_input.mean(axis=1,keepdim=True)
        
        if prep_batch:
            #print('prep batch.....')
            batch = self.prep_batch(batch=batch,prompt_embed=prompt_embed)
        
        else:
            assert 'inputs_embeds' in batch, 'inputs_embeds not in batch'
        
        #hzx added
        if self.shadowembed is not None:
            input_embeds_shadow = self.shadowembed(batch=batch)
            input_embeds = self.embedder(batch=batch)
            
            outputs_shadow = self.shadowdecoder(inputs=input_embeds_shadow,hidden=input_embeds.mean(axis=1,keepdim=True).permute(1,0,2))
            
            batch['inputs_embeds'] = input_embeds
            outputs = self.decoder(batch=batch, output_shadow=outputs_shadow['outputs'])
        ...
        #hzx added
        if prompt_input_var is not None:
            outputs['prompt_var'] = prompt_input_var #torch.exp(prompt_input_var/2)#.mean(axis=1)
            outputs['prompt_mu'] = prompt_input_mu
        else:
            outputs['prompt_var'] = None
            outputs['prompt_mu'] = None
        
        outputs['mask_ratios'] = batch['mask_bold'] #hzx added
