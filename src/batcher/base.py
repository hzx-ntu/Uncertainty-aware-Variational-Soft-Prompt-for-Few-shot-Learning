...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        #hzx added
        if self.is_test:
           self.sample_keys.append('__key__')
    ...
    def __getitem__(self, idx):
        ...
        #hzx added
        if self.is_test:
           self.sample_names.append(sample['__key__'][0])
...
class BaseBatcher:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.is_mask_bold = False #hzx added
    ...
    def _make_dataloader(
        ...
        num_workers: int = 0,
        is_test: bool = False,
        #is_mask_bold: bool = False
        ) -> Generator[Dict[str, torch.tensor], None, None]:
        
        #hzx added
        if is_test:
           n_shuffle_shards = None
           n_shuffle_samples = None
           repeat = False
        
        #self.is_mask_bold = is_mask_bold #hzx added
    ...
    def dataset(
        ...
        n_shuffle_samples: int = 1000,
        num_workers: int = 0,
        is_test: bool = False,  #hzx added
        is_mask_bold: bool = False #hzx added
    ...
    def preprocess_sample(
        ...
        for key, value in sample.items():
            if key == "bold.pyd":
                ...
                #hzx added to mask the bold randomly in training
                if self.is_mask_bold and random.random()>0.5:
                   msk_rtio = random.choice([0.5,0.3,0.1])
                   msk_num = int(msk_rtio*seq_len)
                   if msk_num ==0: 
                      out['mask_bold'] = 0
                   else: 
                     for msk_i in range(msk_num):
                         mask_id = random.randint(0, seq_len)
                         bold[mask_id,:] = 0
                     if seq_len <= 3:
                        out['mask_bold'] = 1 
                     else:
                        out['mask_bold'] = msk_rtio * 2
