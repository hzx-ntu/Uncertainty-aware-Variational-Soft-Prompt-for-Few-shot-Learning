...
from src.embedder import make_embedder, SoftPrompt, PromptTransformerModule, SoftPromptVAE, SoftPromptLSTM
...
def read_tar(tar_file):
    tar = tarfile.open(tar_file)
    sample_list=[]
    for member in tar.getmembers():
        sample_idx = member.name.split('.')[0].split('_')[-1]
        if sample_idx not in sample_list:
...
def train(config: Dict=None) -> Trainer:
    ...
    config['do_train'] = False #hzx added
    ...
    #hzx added
    test_sample_num=0
    for test_tar in test_tarfile_paths:
        test_sample_num += read_tar(test_tar)
        
    print('test sample number : ', test_sample_num)
    ...
    if test_dataset is not None:
        ...
        trainer.is_promptembed = False #hzx added: save the prompt_embeds
        trainer.is_promptembed_vae = False #hzx added: save the prompt_vars
        trainer.is_masked =  False #hzx added: save the masked ratio
        
        #if config["mask_bold"]:   
        batcher.is_mask_bold = True #hzx added
        
        #print('start testing.....')
        test_prediction = trainer.predict(test_dataset)

        if trainer.is_promptembed_vae: 
        
            #hzx added save prompt
            prompt_vars = [p_e.detach().cpu().numpy() for p_e in trainer.prompt_vars]
            prompt_vars = np.vstack(prompt_vars)
            prompt_vars = np.abs(prompt_vars)
            prompt_vars_norm = np.mean(prompt_vars,axis=-1)
            
            np.save(
                os.path.join(
                    config["log_dir"],
                    'test_prompt_vars_norm_0.npy'
                ),
                prompt_vars_norm
            )
        
        if trainer.is_masked: 
        
            #hzx added save prompt
            masked_ratios = [m_r.detach().cpu().numpy() for m_r in trainer.masked_ratio]
            masked_ratios = np.vstack(masked_ratios)
            
            np.save(
                os.path.join(
                    config["log_dir"],
                    'test_masked_ratios_0.npy'
                ),
                masked_ratios
            )
        ...
        #hzx added
        with open(os.path.join(config["log_dir"], 'test_names_1.txt'), "w") as output:
             for sample_name in str(test_dataset.sample_names[0:test_prediction.label_ids.shape[0]]):
...
def make_model(model_config: Dict=None):
    ...
    decoder = make_decoder(
        ...
        is_prompt_residual=model_config["is_prompt_residual"],
    )
    ...
    #hzx added
    softprompt = None
    if model_config["soft_prompt"]:
       print('Using soft prompting....')
       softprompt = SoftPrompt()
    
    #hzx added
    if model_config["soft_prompt_transformer"]:
       print('Using soft prompting-transformer....')
       softprompt = PromptTransformerModule()
    
    #hzx added
    if model_config["soft_prompt_vae"]:
       print('Using soft prompting-vae....')
       softprompt = SoftPromptVAE()
    
    #hzx added
    if model_config["soft_prompt_lstm"]:
       print('Using soft prompting-lstm....')
       softprompt = SoftPromptLSTM()
    
    
    #hzx added
    if model_config["shadow_module"]:
        print('Using shadow module....')
        #hzx added
        shadowembed = make_embedder(
            training_style=model_config["training_style"],
            architecture=model_config["architecture"],
            in_dim=model_config["parcellation_dim"],
            embed_dim=model_config["embedding_dim"],
        ...
        #hzx added
        shadowdecoder = make_decoder(
            architecture='shadowlstm',
            num_hidden_layers=model_config["num_hidden_layers"],
            embed_dim=model_config["embedding_dim"],
            num_attention_heads=model_config["num_attention_heads"],
            n_positions=model_config["n_positions"],
    ...
    model = Model(
        ...
        shadowembed=shadowembed,
        shadowdecoder=shadowdecoder,
        is_prompt_norm=model_config["is_prompt_norm"]
    )
    ...
    #hzx added
    if model_config["freeze_cls_embed"]:
        for param_name,param in model.embedder.named_parameters():
            if 'cls_embed' in param_name:
                param.requires_grad = False

    if model_config["freeze_decoder"]:
        #for param in model.decoder.parameters():
        for param_name,param in model.decoder.named_parameters():
            if 'pooler_layer' not in param_name and 'decoding_head' not in param_name and 'residual_layer' not in param_name:
                param.requires_grad = False
...
def get_config(args: argparse.Namespace=None) -> Dict:
    ...
    args.log_dir = os.path.join(args.log_dir) #hzx added
...
def get_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to freeze the original pretrained model and add soft prompt module '
             '(default: True). '
             'If "False", the soft prompt module will not be involved and whole model will be fine-tuned '
    )
    #hzx added
    parser.add_argument(
        '--soft-prompt-transformer',
        metavar='BOOL',
        default='False',
        type=str,
        choices=('True', 'False'),
        help='whether or not to freeze the original pretrained model and add soft prompt transformer module '
             '(default: True). '
             'If "False", the soft prompt transformer module will not be involved and whole model will be fine-tuned '
    )
    #hzx added
    parser.add_argument(
        '--soft-prompt-vae',
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to freeze the original pretrained model and add soft prompt VAE module '
             '(default: True). '
             'If "False", the soft prompt VAE module will not be involved and whole model will be fine-tuned '
    )
    #hzx added
    parser.add_argument(
        '--soft-prompt-lstm',
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to freeze the original pretrained model and add soft prompt LSTM module '
             '(default: True). '
             'If "False", the soft prompt LSTM module will not be involved and whole model will be fine-tuned '
    )
    #hzx added
    parser.add_argument(
        '--is-prompt-norm',
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to normolize the weight of soft prompt'
             '(default: True). '
             'If "True", the weight of soft prompt module will be normolized'
    )
    #hzx added
    parser.add_argument(
        '--is-prompt-residual',
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to pass the soft prompt to the decoder header '
             '(default: True). '
             'If "False", the soft prompt module will not be passed to the decoder header '
    )
    #hzx added
    parser.add_argument(
        '--freeze-cls-embed',
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to freeze the cls-embed '
             '(default: False). '
             'If "True", the cls-embed will not be updated during the training '
    )
    ...
    parser.add_argument(
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to use the shadow module '
             '(default: False). '
             'If "True", the shadow module will not be added '
    )
    ...
    parser.add_argument(
        ...
        type=str,
        choices=('True', 'False'),
        help='whether or not to mask the input '
             '(default: False). '
             'If "True", the input will be masked randomly '
    )
    #hzx added
    parser.add_argument(
        '--loss-hpara1',
        ...
        default=0.5,
        type=float,
        help='the parameter to balance the CE loss and the KL loss '
             '(default: 0.5)'
    )
    #hzx added
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help='the parameter to balance the masked uncertainty in the KL loss '
             '(default: 0.5)'
    )
    #hzx added
    parser.add_argument(
        ...
        default=0.1,
        type=float,
        help='the parameter to balance the reparameterization '
             '(default: 0.1)'
    )
