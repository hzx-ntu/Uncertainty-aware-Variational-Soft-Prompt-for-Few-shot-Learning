...
def train(config: Dict=None) -> Trainer:
    ...
    if test_dataset is not None:
        print('start testing.....')
        if config["soft_prompt_vae"]:
           trainer.model.softprompt.is_test = True
        
        batcher.is_mask_bold = False #hzx added
...
def make_model(model_config: Dict=None):
    ...
    embedder = make_embedder(
        ...
        loss_para_1=model_config["loss_hpara1"], #hzx added
        loss_para_2=model_config["loss_hpara2"], #hzx added
    )
    ...
    if model_config["soft_prompt_vae"]:
       ...
       softprompt = SoftPromptVAE(vae_para=model_config["vae_para"])
